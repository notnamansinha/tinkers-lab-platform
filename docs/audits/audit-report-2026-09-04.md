...
2. **CRITICAL — A live Figma personal access token exists in git history.** `figd_REDACTED` was committed in `fetch-figma-svgs.js` (first in `f8d2f26`, removed in `eea0ecb` "chore: move figma token to env variable"). It is still reachable in history. Figma PATs grant full account-scope API access. Revoke the token now; scrub history and purge GitHub cached data.
