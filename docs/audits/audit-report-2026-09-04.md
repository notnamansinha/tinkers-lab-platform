...
### §6 Testing
Unit + component: `src/lib/__tests__/` (utils incl. `isSafeStorageUrl`/`debugLog`, teamMembers, roles), `src/services/firebase/__tests__/` (auth with privilege-stripping, bookings, toolCheckouts, overdue), `src/routes/__tests__/guards.test.tsx` (jsdom) — **95 tests across 8 files** (`npm test`), all green. Emulator suite (`npm run test:rules`, boots firestore/storage/functions/auth): **375 tests** — 240 Firestore-rule attack vectors, 49 Storage-rule cases, 86 Cloud Function tests (validation, transaction isolation incl. concurrent double-booking, rate limits, triggers). CI gates on lint/build/unit/emulator-suite. Remaining QA-dependent gaps: browser-level empty/network/loading/error states and manual Lighthouse checks — see `docs/development/TESTING.md` §4.
...
All in-repo findings are implemented and committed (see `git log --oneline` from `d576080`). Verification: app build , functions build , 95 unit+component tests , 375 emulator tests (Firestore+Storage rules attack vectors + all Cloud Functions) typecheck  (emulator run requires Java — runs in CI), `npm audit` root = 0 vulns.

| ID | Status | Notes |
|----|--------|-------|
| F-01 | **Done (repo) /  user action required** | Full history scrubbed + force-pushed to `main`/`fixes`/`testing` (ruleset temporarily disabled, restored). Local object store verified token-free. **Remaining:** (1) REVOKE the Figma token in Figma settings — the leaked string is inert only after revocation; (2) request GitHub support purge of stale closed-PR head refs `refs/pull/1/head` (728403961d) and `refs/pull/2/head` (cfa62478ce) which still contain the old file (not deletable via API). |
