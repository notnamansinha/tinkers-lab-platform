...
6. Seed equipment: `npx tsx scripts/seedEquipment.ts` (or equivalent) with a staff account.
7. `firebase deploy --only hosting` — ship the app.
8. If migrating existing top-level `bookings`/`toolCheckouts` data: run `scripts/migrateToSubcollections.ts` **before** switching over (requires `firebase-admin` + a service account — never commit the service account file).
...
The app connects to emulators automatically when `import.meta.env.DEV` and `VITE_USE_EMULATORS === 'true'`. Firestore uses persistent local cache for offline support & reduced reads.

> ⚠️ The rules were authored but not yet emulator-tested (no Java in the original dev env). Test against a staging project before a production cutover.
